...
func (amazonDEImporter) Info() model.LedgerEmailImporterInfo {
	return model.LedgerEmailImporterInfo{
...
func (amazonDEImporter) Parse(parsed ParsedEmail) ([]ParsedOrder, []string, error) {
	...
	var current *model.LedgerEmailOrderItem
	...
	for _, line := range lines {
		...
		case strings.HasPrefix(line, "* "):
			flushCurrent()
			current = &model.LedgerEmailOrderItem{Name: normalizeInlineWhitespace(strings.TrimPrefix(line, "* ")), Quantity: 1}
...
func (payPalDEImporter) Info() model.LedgerEmailImporterInfo {
	return model.LedgerEmailImporterInfo{
