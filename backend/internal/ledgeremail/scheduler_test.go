...
func TestAccountDue(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			account := model.LedgerEmailAccount{LastScanAt: tc.lastScanAt}
...
func TestScanStatusMessage(t *testing.T) {
	msg := scanStatusMessage(model.LedgerEmailScanResult{EmailsScanned: 12, OrdersNew: 3, OrdersLinked: 2})
